...
def eval(prediction_file, gold_file):
    ...
    # gold can be json or jsonl
    with open(gold_file, 'r', encoding='utf-8') as f:
        if gold_file.endswith('.jsonl'):
            gold = [json.loads(line) for line in f if line.strip()]
        else:
            gold = json.load(f)

    has_evidence = isinstance(prediction, dict) and ('evidence' in prediction)
    ...
    for dp in gold:
        ...
        if has_evidence:
            ...
            evi_em, evi_prec, evi_recall = 1.0, 1.0, 1.0
    ...
    if not has_evidence:
        metrics.pop('evi_em', None)
        metrics.pop('evi_f1', None)
        metrics.pop('evi_prec', None)
        metrics.pop('evi_recall', None)
